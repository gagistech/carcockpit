...
gauge::gauge(utki::shared_ref<ruis::context> c, all_parameters p)
    : widget(std::move(c), {.widget_params = std::move(p.widget_params)}),
      ...
      fraction_widget(this->context, {}), params(std::move(p.params)) {}
...
void gauge::on_lay_out() {
  ...
  auto scale =
      (std::max(this->rect().d.x(), this->rect().d.y()) / 2) / arm_length;

  this->arrow_tex =
      this->params.arrow.get().get(arrow_dim * scale).to_shared_ptr();

  if (this->params.shadow) {
    // TRACE(<< "this->shadow->dims() * scale = " << this->shadow->dims() *
    // scale << std::endl)
    this->shadow_tex =
        this->params.shadow->get(this->params.shadow->dims() * scale)
            .to_shared_ptr();
...
void gauge::render(const matrix4 &matrix) const {
  ...
  matrix4 mmm;
  mmm.set_identity();
  mmm.rotate(-(this->params.start_angle_rad +
               (this->params.end_angle_rad - this->params.start_angle_rad) *
                   this->get_fraction()));
  {
  ...
  if (this->shadow_tex && this->shadow_tex->dims.is_positive()) {
    auto arrow_fraction =
        this->arrow_tex->dims.x() / this->shadow_tex->dims.x();
    ...
    m.translate(-(1 - this->params.arm_fraction) * arrow_fraction -
                    (1 - arrow_fraction) / 2,
                -half);
