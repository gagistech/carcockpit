...
car_widget::car_widget(utki::shared_ref<ruis::context> context,
  ...
  std::array<ruis::vector3, 36> cube_pos = {
      {ruis::vector3(-0, -0, 0),  ruis::vector3(0, -0, 0),
       ruis::vector3(-0, 0, 0),   ruis::vector3(0, -0, 0),
       ruis::vector3(0, 0, 0),    ruis::vector3(-0, 0, 0),
       ruis::vector3(0, -0, 0),   ruis::vector3(0, -0, -0),
       ruis::vector3(0, 0, 0),    ruis::vector3(0, -0, -0),
       ...
       ruis::vector3(0, -0, -0),  ruis::vector3(0, -0, 0)}};
  auto pos_vbo =
      this->context.get().renderer.get().factory->create_vertex_buffer(
          utki::make_span(cube_pos));
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
  ...
  // clang-format on
  auto cube_indices =
      this->context.get().renderer.get().factory->create_index_buffer(
          utki::make_span(indices));

  this->cube_vao =
      this->context.get()
          .renderer.get()
          .factory
          ->create_vertex_array({pos_vbo, tex_vbo}, cube_indices,
                                ruis::render::vertex_array::mode::triangles)
          .to_shared_ptr();

  this->tex = this->context.get()
                  .loader.load<ruis::res::texture>("tex_sample")
                  .to_shared_ptr();
  ...
  this->tex_test = this->context.get()
                       .loader.load<ruis::res::texture>("tex_test")
                       .to_shared_ptr();

  this->tex_car_diffuse =
      this->context.get()
          .loader.load<ruis::res::texture>("tex_car_diffuse1")
          .to_shared_ptr();
  this->tex_car_normal = this->context.get()
                             .loader.load<ruis::res::texture>("tex_car_normal1")
                             .to_shared_ptr();
  this->tex_car_roughness =
      this->context.get()
          .loader.load<ruis::res::texture>("tex_car_roughness1")
          .to_shared_ptr();

  this->tex_rust_diffuse =
      this->context.get()
          .loader.load<ruis::res::texture>("tex_car_diffuse")
          .to_shared_ptr();
  this->tex_rust_normal = this->context.get()
                              .loader.load<ruis::res::texture>("tex_car_normal")
                              .to_shared_ptr();
  this->tex_rust_roughness =
      this->context.get()
          .loader.load<ruis::res::texture>("tex_car_roughness")
          .to_shared_ptr();
  ...
  std::shared_ptr<ModelOBJ> lamba_right_model_obj =
      std::make_shared<ModelOBJ>();
  ...
  this->light_vao =
      this->context.get()
          ...
          .factory
          ->create_vertex_array(
              {light_vbo_positions, light_vbo_texcoords, light_vbo_normals,
               light_vbo_tangents, light_vbo_bitangents},
              light_vbo_indices, ruis::render::vertex_array::mode::triangles)
          .to_shared_ptr();
  ...
  this->car_vao =
      this->context.get()
          ...
          .factory
          ->create_vertex_array(
              {car_vbo_positions, car_vbo_texcoords, car_vbo_normals,
               car_vbo_tangents, car_vbo_bitangents},
              car_vbo_indices, ruis::render::vertex_array::mode::triangles)
          .to_shared_ptr();
  ...
  this->vao_lamba_l =
      this->context.get()
          ...
          .factory
          ->create_vertex_array(
              {lcar_vbo_positions, lcar_vbo_texcoords, lcar_vbo_normals,
               lcar_vbo_tangents, lcar_vbo_bitangents},
              lcar_vbo_indices, ruis::render::vertex_array::mode::triangles)
          .to_shared_ptr();
  ...
  this->vao_lamba_r =
      this->context.get()
          ...
          .factory
          ->create_vertex_array(
              {rcar_vbo_positions, rcar_vbo_texcoords, rcar_vbo_normals,
               rcar_vbo_tangents, rcar_vbo_bitangents},
              rcar_vbo_indices, ruis::render::vertex_array::mode::triangles)
          .to_shared_ptr();
...
void car_widget::update(uint32_t dt) {
  ...
  this->rot =
      // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
      (ruis::quaternion().set_rotation(r4::vector3<float>(0, 1, 0).normalize(),
                                       2.0f * 3.1415926f *
                                           float(get_fraction()))) *
      (ruis::quaternion().set_rotation(r4::vector3<float>(0, 1, 0).normalize(),
                                       0.1f * ft));
  ...
  {
    camera_position +=
        (camera_attractor - camera_position) * fdt / camera_transition_duration;
    ruis::vec3 remains = camera_attractor - camera_position;
    ruis::real l2 = remains.x() * remains.x() + remains.y() * remains.y() +
                    remains.z() * remains.z();
...
void car_widget::render(const ruis::matrix4 &matrix) const {
  ...
  projection.set_perspective(
      3.1415926535f / 3.f, this->rect().d[0] / this->rect().d[1], 0.1f, 20.0f);
  view.set_identity();
  ...
  ruis::vec4 light_pos_view =
      view * light_pos; // light position in view (camera) coords
  ...
  //  					this->tex_rust_normal->tex(), this->tex_rust_roughness->tex(),
  //  light_pos_view, light_int);
  ...
  // phong_s->render(*this->vao_lamba_r, mvp, modelview,
  // this->tex_car_diffuse->tex(), light_pos_view, light_int);
  phong_s->render(*this->light_vao, mvp_monkey, modelview_monkey,
                  this->tex_test->tex(), light_pos_view, light_int);

  advanced_s->render(*this->vao_lamba_l, mvp, modelview, projection,
                     this->tex_car_diffuse->tex(), this->tex_car_normal->tex(),
